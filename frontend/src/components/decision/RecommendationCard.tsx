...
export function RecommendationCard({ recommendation }: RecommendationCardProps) {
  ...
  const titleLabel = language === 'Swahili' ? 'Utekelezaji wa Hatua' : recommendation.actionTitle
  const actorLabel =
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <h3 className="mt-3 text-base font-bold text-slate-900">{actorLabel}</h3>
          <p className="mt-2 text-sm font-semibold text-slate-800">{titleLabel}</p>
          <p className="mt-2 text-sm leading-6 text-slate-700">{recommendation.actionDetails}</p>
        </div>
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        <span className={['inline-flex rounded-full px-3 py-1 text-xs font-semibold uppercase tracking-[0.16em]', urgencyTone(recommendation.urgency)].join(' ')}>
      ...
      >
        <span>{language === 'Swahili' ? 'Muktadha na Ushahidi' : 'Reasoning / Evidence'}</span>
        <ChevronDown
          className={[
      ...
      {isExpanded ? (
        <div className="mt-3 space-y-3 rounded-2xl bg-slate-50 p-4 text-sm leading-6 text-slate-600">
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-3">
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">Supporting Evidence</p>
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-3">
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">Estimated Impact If Delayed</p>
            <p className="mt-2 text-sm text-slate-700">{recommendation.estimatedImpactIfDelayed}</p>
          </div>
        </div>
      ) : null}

      <div className="mt-4 flex items-center justify-between gap-3">
        <span className="text-xs uppercase tracking-[0.2em] text-slate-400">Status: {recommendation.status}</span>
        <Button variant={isApproved ? 'secondary' : 'primary'} onClick={() => approveAction(recommendation.id)} disabled={isApproved}>
